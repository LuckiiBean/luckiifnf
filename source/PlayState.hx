...
import flixel.group.FlxGroup.FlxTypedGroup;package;
...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch(SONG.song.toLowerCase())
		{
			case 'pest' | 'invader' | 'right-there':
			{
				defaultCamZoom = 0.70;
					curStage = 'makeout';
					var bg:FlxSprite = new FlxSprite(-600, -400).loadGraphic(Paths.image('grassback'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);
					
					var bg:FlxSprite = new FlxSprite(-600, -25).loadGraphic(Paths.image('grasstrees'));
					bg.antialiasing = true;
					bg.scrollFactor.set(1, 0.9);
					bg.active = false;
					add(bg);

					var stageFront:FlxSprite = new FlxSprite(-650, 290).loadGraphic(Paths.image('grassfront'));
					...
					cameos = new FlxSprite(-300, 120);
					cameos.frames = Paths.getSparrowAtlas('grasspeople');
					cameos.animation.addByPrefix('cameos', 'BGcameos', 24, false);
					cameos.antialiasing = true;
						cameos.scrollFactor.set(1, 1);
						cameos.setGraphicSize(Std.int(cameos.width * 1));
					cameos.updateHitbox();
					add(cameos);
			}
	...
	*/
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'makeout':
				cameos.animation.play('cameos', true);
...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'peset':
				dialogue = CoolUtil.coolTextFile(Paths.txt('pest/pestDialogue'));
		}
